...
5. **Language-specific patterns to check** (apply whichever is relevant to the language in scope):
   ...
   - **Cordova (JavaScript / Hybrid)**: `InAppBrowser` loading user-controlled URLs without an allowlist (`allow-navigation` in `config.xml`), `cordova.exec` calling native plugins with unsanitized JavaScript input, Content Security Policy missing or allowing `unsafe-inline` / `unsafe-eval` in `config.xml` or `index.html`, `<access origin="*" />` in `config.xml` permitting requests to any external domain, JavaScript-to-native bridge (`cordova.exec`) exposed to content from external origins loaded in the WebView, hardcoded API endpoints, tokens, or secrets in JavaScript source under `www/`, `localStorage` / `sessionStorage` storing sensitive data (auth tokens, PII) accessible to any script in the WebView context, missing `allow-navigation` restrictions enabling open redirect within the WebView, Cordova plugins with overly broad Android permissions in `plugin.xml` or `AndroidManifest.xml`, `eval` or `new Function()` used to execute dynamically constructed strings
